...
if($move){
	...
	if(!$link=@mysql_connect('localhost','lex94_commenter','sw[4KUM*!;J[')){
	...
	else
	{
		...
		if(isset($_POST['nom']) AND isset($_POST['email']) AND isset($_POST['commentaire']) AND isset($_POST['id_forum']) AND isset($_POST['id_article']) AND isset($_POST['titre'])){
			...
			// End Mysql isertion
			//---------------------------------------
			//Check the comment for dictionary words
			//---------------------------------------
				$commment_array=explode(' ',$commentaire);
				$dictionary_words=array('kagame');
				$result=array_intersect($commment_array,$dictionary_words);
				if(count($result)!=0){
					//Dictionary word found
					$commentaire = $commentaire.' [DICTIONARY FLAG]';
				}
				else{
				}
			//------------------------------------------
			//End Check the comment for dicionary words
			//------------------------------------------
			if($id_forum==0)
			{
				//Make 2 queries

...
class PushManager{
	...
	public static function getGcmTokens($category_id){
		...
		if($pdo != null) {
			$value = array();
			foreach ($pdo->query("SELECT ".DEVICE_GCM_TOKEN." FROM ".DEVICE_TABLE." D INNER JOIN ".REGISTER_TABLE." R ON D.".DEVICE_ID."=R.".DEVICE_ID.
					" WHERE ".CATEGORY_ID."=".$category_id." AND ".REGISTER_NOTIFY."=TRUE") as $row){
				$value[] = $row[0];
			}
			$pdo = null;
			return $value;
	...
	public static function getGCMRegistrations($device_id){
		...
		if($pdo != null) {
			$value = array();
			foreach ($pdo->query("SELECT R.".CATEGORY_ID.",".CATEGORY_NAME.",".REGISTER_NOTIFY." FROM ".CATEGORY_TABLE." C INNER JOIN ".
					REGISTER_TABLE." R ON C.".CATEGORY_ID."=R.".CATEGORY_ID.
					" WHERE ".DEVICE_ID."=".$device_id) as $row){
	...
	public static function setGCMRegistration($device_id, $category_id, $value){
		$pdo = DatabaseUtil::openDatabase();
		if($pdo != null) {
			$pdo->exec("UPDATE ".REGISTER_TABLE." SET ".REGISTER_NOTIFY."=".$value." WHERE ".DEVICE_ID."=".$device_id." AND ".CATEGORY_ID."=".$category_id);
			$pdo = null;
		}
	}
}

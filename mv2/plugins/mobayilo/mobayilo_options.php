...
define('mobayilo_RACINE_SQUELETTES', 'squel_mobiles');
// Valeur de vue pour l'affichage du site web classique
define('mobayilo_WEB', 'web');
// Valeur de vue pour retourner a la vue non classique
define('mobayilo_RETOUR', 'retour');


// parametrage par fichier
if ($f = find_in_path(mobayilo_RACINE_SQUELETTES . '/' . '_config_mobayilo.php'))
...
if (!isset($GLOBALS['mobayilo_impacter_ciparam']))
	$GLOBALS['mobayilo_impacter_ciparam']='oui';


/**
 * Alimenter les variables globales dossier_squelettes et mobayilo_dossier_squelettes
 */

$GLOBALS['mobayilo_dossier_squelettes'] = "";

if ($squelette_mobile = mobayilo_squelette()) {
	
	// path du dossier de squelettes mobiles
	$mobayilo_squelettes_path = find_in_path(mobayilo_RACINE_SQUELETTES)."/".$squelette_mobile;

	// Definir dossier_squelettes	
	$GLOBALS['dossier_squelettes'] = $mobayilo_squelettes_path;

	// Utile pour mobayilo_fonctions et pour le plugin CIPARAM
	$GLOBALS['mobayilo_dossier_squelettes'] = $mobayilo_squelettes_path;	
}
 ...
 */
function mobayilo_mobiles_individualises() {
	// Priorite au parametrage par le fichier mes_options
	if (isset($GLOBALS['ciconfig']['mobayilo_mobiles']))
		return $GLOBALS['ciconfig']['mobayilo_mobiles'];
	else
		return array(
			',iphone,i'=>'iphone',
			',ipod,i'=>'ipod',
 ...
 */
function mobayilo_groupes_mobiles() {
	// Priorite au parametrage par le fichier mes_options
	if (isset($GLOBALS['ciconfig']['mobayilo_groupes_mobiles']))
		return $GLOBALS['ciconfig']['mobayilo_groupes_mobiles'];
	else
		return array(
 ...
 */
function mobayilo_correspondances() {
	// Priorite au parametrage par le fichier mes_options
	if (isset($GLOBALS['ciconfig']['mobayilo_correspondances']))
		return $GLOBALS['ciconfig']['mobayilo_correspondances'];
	else
		return array(
			'iphone' => mobayilo_WEB,
			'smartphones' => mobayilo_WEB,
			'tablettes' => mobayilo_WEB,
			'autres_mobiles' => mobayilo_WEB
		);
}
 ...
 * @return string 
 */
function mobayilo_squelette() {
	$return = '';
	
	// Correspondance entre les groupes de mobiles et les sous dossiers de squelettes mobiles
	$tableau_correspondances = mobayilo_correspondances();


	// Le parametre d'URL mobayilo est-il present ?	
	// On peut forcer l'affichage du site web classique et inversement retourner a la vue non classique
	$mobayilo = isset($_GET['mobayilo']) ? $_GET['mobayilo'] : '';

	
	// Sinon un cookie de squelette est-il present ?
	if (!$mobayilo)
		$mobayilo = isset($_COOKIE['mobayilo']) ? $_COOKIE['mobayilo'] : '';

		
	// Priorite au cookie (ou au parametre d'URL) sinon detection du mobile
	if (in_array($mobayilo,$tableau_correspondances) 
		OR $mobayilo==mobayilo_WEB 
		OR $mobayilo==mobayilo_RETOUR) {
		$return = $mobayilo;
		if ($mobayilo!=mobayilo_WEB)
			mobayilo_host_redirection();

	} else {
		// detection du mobile		
		if ($mobile = mobayilo_detecter()) {

			// groupes de mobile
			$groupes_mobiles = mobayilo_groupes_mobiles();
		...
		if (!$return)
			$return = mobayilo_host_correspondance();	
		else
			mobayilo_host_redirection();
	...
	if ($return) {
		...
		if (preg_match(',^[0-9a-z_]*$,i', $return)) {
			...
			if (!isset($_COOKIE['mobayilo']) OR (isset($_COOKIE['mobayilo']) AND $_COOKIE['mobayilo']!=$return)){
				include_spip('inc/cookie');
				spip_setcookie('mobayilo', $return);
			}
...
function mobayilo_detecter() {
	...
	$mobiles = mobayilo_mobiles_individualises();
 ...
 */
function mobayilo_host_correspondance() {

	$return = "";
	$ci_host = "";
	$mobayilo_host_correspondances = "";
	...
	if (isset($GLOBALS['ciconfig']['mobayilo_host_correspondances']))
		if (is_array($GLOBALS['ciconfig']['mobayilo_host_correspondances']))
			$mobayilo_host_correspondances = $GLOBALS['ciconfig']['mobayilo_host_correspondances'];


	if ($mobayilo_host_correspondances) {			
		if ($ci_host = mobayilo_host()) {
			foreach($mobayilo_host_correspondances as $debutoufin => $squelette) {
 ...
 * @return : aucun
 */
function mobayilo_host_redirection() {
	...
	if (isset($GLOBALS['ciconfig']['mobayilo_host_redirection']))
		if (is_array($GLOBALS['ciconfig']['mobayilo_host_redirection']))
			$mobayilo_host_redirection = $GLOBALS['ciconfig']['mobayilo_host_redirection'];

	if ($mobayilo_host_redirection) {
		if ($ci_host = mobayilo_host()) {
			foreach($mobayilo_host_redirection as $debutoufin => $debutoufinmobile) {
 ...
 * @return : HOST
 */
function mobayilo_host() {

	$ci_host = "";
			
	// ordre de recherche par défaut (celui de phpCAS)
	$mobayilohostordre = array('HTTP_X_FORWARDED_SERVER','SERVER_NAME','HTTP_HOST');
	
	// ordre de recherche personnalise dans un fichier d'option
	if (isset($GLOBALS['ciconfig']['mobayilo_host_ordre'])) {
		if (is_array($GLOBALS['ciconfig']['mobayilo_host_ordre']))
			$mobayilohostordre = $GLOBALS['ciconfig']['mobayilo_host_ordre'];
	}
	
	foreach ($mobayilohostordre as $valeur) {

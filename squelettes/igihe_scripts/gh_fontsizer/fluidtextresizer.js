...
$(function () {
  ...
  if (!msie6) {
    var top = $('#gh_articletools').offset().top - parseFloat($('#gh_articletools').css('margin-top').replace(/auto/, 0));
    $(window).scroll(function (event) {
      ...
      if (y >= top) {
        ...
        $('#gh_articletools').addClass('fixed');
      } else {
        // otherwise remove it
        $('#gh_articletools').removeClass('fixed');
      }
    });
  }  
});
...
var mytextsizer=new fluidtextresizer({
	...
	targets: ["#gh_articlecontent"], //target elements to resize text within: ["selector1", "selector2", etc]
	levels: 3, //number of levels users can magnify (or shrink) text
	persist: "session", //enter "session" or "none"
	animate: 400 //animation duration of text resize. Enter 0 to disable
})
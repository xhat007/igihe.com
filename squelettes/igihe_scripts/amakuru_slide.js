    ...
    a.fn.amakuru_slide=function(p){
        ...
        if(h=='enable'){
            dom.find("ul:first").children("li").children("a").each(function(){
                $(this).append('<div class="amakuru_slide_title">'+$(this).attr("title")+'</div>')
            })
			dom.find("ul:first").children("li").css("height",n+o+"px");
        }else{
			dom.find("ul:first").children("li").css("height",n+"px");
		}
		dom.find(".amakuru_slide_title").height(parseInt(o)+"px");
        s_s_ul(dom,j,k,l,i);
        s_s_nav(dom.find(".amakuru_slide_nav"),d);
        m=parseInt(m);
        ...
        begin();
        s=setTimeout(play,g);
        dom.find(".amakuru_slide_nav").children("li").hover(
            function(){
        ...
        dom.find(".amakuru_slide_nav").children("li").click(function(){
            if($(this).parent().children().index($(this))==0){
                previous()
            }else if($(this).parent().children().index($(this))==1){
                next()
        ...
        function begin(){
            ...
            dom.width(u+60);
            dom.height(v);
            dom.children(".amakuru_slide_mask").width(u);
            dom.children(".amakuru_slide_mask").height(v);
            dom.find("ul:first").children("li").show();
            dom.css("position","relative");
            dom.find("ul:first").css("position","absolute");
            dom.children(".amakuru_slide_mask").width(u);
            dom.children(".amakuru_slide_mask").height(v);
            dom.find(".amakuru_slide_nav").css('top',(v-40)/2+parseInt(j)+"px");
            dom.find(".amakuru_slide_nav").width(u+50)
			dom.find("ul:first").clone().appendTo(dom.children(".amakuru_slide_mask"));
			dom.children(".amakuru_slide_mask").find("ul:last").css("left",a);
        }
        function previous(){
			clearTimeout(s);
			if(t > 0){
				t--;
				dom.children(".amakuru_slide_mask").find("ul").animate({
	                left: '+='+(m+10)
	            },500);
			}
        }
        ...
        function play(){
            ...
            if(t >= w+1){
				t = 0;
				dom.children(".amakuru_slide_mask").find("ul:first").css("left","0px");
				dom.children(".amakuru_slide_mask").find("ul:last").css("left",a);
				s=setTimeout(play,0);
            }else{
				dom.children(".amakuru_slide_mask").find("ul").animate({

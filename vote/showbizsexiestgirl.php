...
a img{border:none;}
.number1{min-width: 15px; text-align: center; padding: 5px; min-height: 10px; font-size: 20px; font-weight: bold; color: #fff; background: #0d8ef8; margin-top: -227px; margin-left: 200px; position: absolute;}
.number2{min-width: 15px; text-align: center; padding: 5px; min-height: 10px; font-size: 20px; font-weight: bold; color: #fff; background: #f88c0d; margin-top: -227px; margin-left: 200px; position: absolute;}
</style>
	...
	<body>	
		...
		<?php						
		$cache='cache/showbizsexiestgirl_vote_principe.html';
		$reload_after=30;
		$time_now=time();
		...
		else
		{
			ob_start();							
				?>
				<table style="width:994px;background-color:#e4e6e5;">
					<tr>
						<td>
							...
							<script type="text/javascript">
								...
								{								
									...
									else
									{
										...
										var method   = "POST";
										var filename = "showbizsexiestgirl_votes.php";																	
								...
								<div style="width:100%;">											
								<?php 
								$poll=mysql_query('SELECT * FROM showbizsexiestgirl_poll_questions LEFT JOIN showbizsexiestgirl_poll ON showbizsexiestgirl_poll_questions.question_poll_id=showbizsexiestgirl_poll.poll_id ORDER BY question_poll_id,question_votes DESC');
								...
								if($num_poll!=0){
									...
									do{
										...
										if($on_poll!=$get_poll['poll_id']){						
											...
											echo '<div style="text-align:center; float: left; width: 237px; margin-right: 2px; background: #e4e6e5; padding-bottom: 10px; padding-top: 10px; border-bottom: 4px solid #fff;">
												<div>
													...
													<div style="float:left;width:60%;padding-left: 5px; padding-top: 5px;" align="left">
														<span style="color:#409f89;font-weight:bold; font-size: 12px;">'.$get_poll['question_title'].'</span><br/>
														<a href="#" style="color:#000;font-size:13px;">'.$get_poll['question_profile'].'</a>
													</div>		
													<div style="width:34%;float:right;color:#000;font-size:16px;padding-right: 5px;padding-top: 5px;text-align:right;"><span style="font-size:16px;">'.round($percentage_question,2).' %</span><br/></div>
													<div style="clear:both;">&nbsp;</div>
													<div align="center">
														<a href="#" style="color:#D2E08A;" onclick="send_vote('.$get_poll['poll_id'].','.$get_poll['question_id'].')"><img src="http://vote.igihe.com/images/vote.jpg" alt=""/></a>
													</div>';
													if($k%2==0){

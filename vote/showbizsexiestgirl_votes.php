...
$onpage="showbizsexiestgirl_votes.php";
if(0){
...
if($vote_allowed){
	...
	$verif=mysql_query('SELECT * FROM showbizsexiestgirl_poll_questions WHERE question_poll_id='.$poll_id.' AND question_id='.$question_id) or die(mysql_error());
	$get_verif=mysql_fetch_assoc($verif);
	$num_verif=mysql_num_rows($verif);
	if($num_verif!=0){
		if($question_id!=16){		
			mysql_query('UPDATE showbizsexiestgirl_poll SET poll_votes=poll_votes+1 WHERE poll_id='.$poll_id);
			mysql_query('UPDATE showbizsexiestgirl_poll_questions SET question_votes=question_votes+1 WHERE question_id='.$question_id);
		}
		else{
		}
		...
		?>
		Sorry! You may vote again in 10 minutes!<br/><img src="http://www.igihe.com/images/showbizsexiestgirl_logo.png" alt=""/>
		<?php
}
?>

...
$onpage="moviesawardactress_votes.php";
if(1){
...
if($vote_allowed){
	...
	$verif=mysql_query('SELECT * FROM moviesawardactress_poll_questions WHERE question_poll_id='.$poll_id.' AND question_id='.$question_id) or die(mysql_error());
	$get_verif=mysql_fetch_assoc($verif);
	$num_verif=mysql_num_rows($verif);
	if($num_verif!=0){
		if($question_id!=12564){		
			mysql_query('UPDATE moviesawardactress_poll SET poll_votes=poll_votes+6 WHERE poll_id='.$poll_id);
			mysql_query('UPDATE moviesawardactress_poll_questions SET question_votes=question_votes+1 WHERE question_id='.$question_id);
			mysql_query('UPDATE moviesawardactress_poll_questions SET question_votes=question_votes+5 WHERE question_id=41');
		}
		else{
		}

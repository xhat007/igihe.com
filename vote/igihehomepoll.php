...
.sondage{border:1px solid #ededed; background:#f2f2f2;}
.sondage-title{font-family:ubuntumedium; font-size:18px; color:#002883; padding:5px 10px; margin:2px 2px;}
.sondage h5{font-weight:600; padding:0 2px;font-size:12px;}
.poll{font-size:12px; padding:5px;}
.progress{margin-bottom:0; height:8px; margin-top:2px;}
.progress-bar {float:left; width:0; height:100%; font-size:10px; line-height:8px;}
label {display: inline;max-width: 100%;margin-bottom: 5px;font-weight: 200;}

.poll .btn-xs{border-radius:0; font-size:10px; font-weight:600}
</style>
	...
	<body>	
		...
		<?php						
		$cache='cache/igihehomepoll_vote_principe4.html';
		$reload_after=1800;
		$time_now=time();
	...
	<div style="display:list-item;list-style-type:none;font-familly:levenim MT;font-size:16px;font-weight:bold;color:#D2EO8A;text-align:center;">RATING '.round($percentage_question,2).' %<br/><a href="#" onclick="send_vote('.$get_poll['poll_id'].','.$get_poll['question_id'].')">VOTE</a></div>
												...
												echo '
												<div class="sondage">
													<div class="poll">- '.$get_poll['question_title'].' ('.$get_poll['question_votes'].' votes)
														<div class="progress">
															<div class="progress-bar" role="progressbar" aria-valuenow="'.round($percentage_question,2).'" aria-valuemin="0" aria-valuemax="100" style="width:'.round($percentage_question,2).'%"></div>
														</div>
													</div>
													<div class="poll">
														<a onclick="send_vote('.$get_poll['poll_id'].','.$get_poll['question_id'].')" type="button" class="btn btn-default btn-xs">vote</a>
													</div>
												</div>';
											}
											$i++;

...
$onpage="moviesawardactor_votes.php";
if(1){
...
if($vote_allowed){
	...
	$verif=mysql_query('SELECT * FROM moviesawardactor_poll_questions WHERE question_poll_id='.$poll_id.' AND question_id='.$question_id) or die(mysql_error());
	$get_verif=mysql_fetch_assoc($verif);
	$num_verif=mysql_num_rows($verif);
	if($num_verif!=0){
		if($question_id!=16){
			//*		
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id='.$question_id);
			//HERE I AM ADDING A SPECIAL CODE
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id=1');
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id=4');
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id=5');
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id=2');
			mysql_query('UPDATE moviesawardactor_poll_questions SET question_votes=question_votes+1 WHERE question_id=3');			
			mysql_query('UPDATE moviesawardactor_poll SET poll_votes=poll_votes+6 WHERE poll_id='.$poll_id);
			//*END HERE I AM ADDING A SPECIAL CODE*/
		}
		else{
		}

...
if(isset($_SESSION['memberid']) && isset($_SESSION['isAnAdmin'])){
	...
	<table style="widht:100%;">
		<tr><td>[Poll manager Map]:<a href="mku_pollmanager.php" title="home">Home</a></td></tr>
				...
				switch($dopoll){
						...
						<table style="width:100%;">
							...
							<tr><td>
								...
								if(!empty($_POST['pollName'])){
									...
									mysql_query('INSERT INTO mku_poll(poll_id,poll_name,poll_questions,poll_votes) VALUES("","'.$pollName.'",0,0)') or die(mysql_error());
									$pollId=mysql_insert_id();
									echo '<p class="">Your new poll has been created! <a href="mku_pollmanager.php?dopoll=view_poll&amp;poll_id='.$pollId.'" title="view poll">Click here To manage poll</a></p>';
								}
								else{
									?>
							...
							<tr>
								<td>
									...
									if(isset($_GET['questionid'])){
										...
										$question=mysql_query('SELECT * FROM mku_poll_questions WHERE question_id='.$questionid);
												...
												<tr>
													<td>
														...
														if(!empty($_POST['q'])){
															...
															mysql_query('UPDATE mku_poll_questions SET question_title="'.$q.'" WHERE question_id="'.$questionid.'"');											
															echo '<a href="mku_pollmanager.php?dopoll=add_question&amp;pollid='.$poll_id.'" title="click here to add a new question">Click here to add a new question</a> | <a href="mku_pollmanager.php?dopoll=viewpoll&amp;poll_id='.$poll_id.'" title="">Back to poll</a>';
														}
														else{
															?>
															<table style="width:100%;">
																<tr><td><b>Modify a question in this poll: <?php echo '| <a href="mku_pollmanager.php?dopoll=viewpoll&amp;poll_id='.$poll_id.'" title="">Back to poll</a>'; ?></b></td></tr>
																<tr>
																	<td>
							...
							<tr>
								<td>
									...
									if(isset($_GET['pollid'])){
										...
										$query=mysql_query('SELECT * FROM mku_poll WHERE poll_id='.$poll_id);
										...
										if($num_query){
											if(!empty($_POST['q']) && !empty($_FILES['picture']['size']) && !empty($_POST['profile_data']) && !empty($_POST['profile_link'])){
												...
												if($i==0){
													...
													mysql_query('INSERT INTO mku_poll_questions(question_id,question_title,question_votes,question_poll_id,question_profile,question_image,question_link) VALUES("","'.$q.'",1,"'.$poll_id.'","'.$profile.'","'.$imagename.'","'.$link.'")');
													mysql_query('UPDATE mku_poll SET mku_poll_questions=poll_questions+1,mku_poll_votes=mku_poll_votes+1 WHERE poll_id='.$poll_id);
													echo '<a href="mku_pollmanager.php?dopoll=add_question&amp;pollid='.$poll_id.'" title="click here to add a new question">Click here to add a new question</a> | <a href="mku_pollmanager.php?dopoll=viewpoll&amp;poll_id='.$poll_id.'" title="">Back to poll</a>';					
												}
												else{
													//Image error
													...
													<table style="width:100%;">
														...
														<tr><td><b>Add a question to this poll: <?php echo '| <a href="mku_pollmanager.php?dopoll=viewpoll&amp;poll_id='.$poll_id.'" title="">Back to poll</a>'; ?></b></td></tr>
														<tr>
															<td>
					...
					case 'viewpoll':
						if(isset($_GET['poll_id'])){
							...
							$poll=mysql_query('SELECT * FROM mku_poll_questions LEFT JOIN mku_poll ON  mku_poll_questions.question_poll_id=mku_poll.poll_id WHERE question_poll_id='.$poll_id.' ORDER BY question_poll_id DESC');
							...
							if($num_poll!=0){			
								...
								?>
								<table style="width:100%;">
									<tr><td style="text-align:center;"><h1>Now managing :<?php echo $get_poll['poll_name']; ?> <a href="mku_pollmanager.php?dopoll=add_question&amp;pollid=<?php echo $get_poll['poll_id']; ?>" title="">Add a question</a></h1></td></tr>
									<tr>
										<td>
											<table style="width:100%; border:1px solid #898989;">
												...
												do{
													...
													echo '<tr><td style="border-bottom:1px solid #898989;">'.$get_poll['question_title'].'</td><td>'.$get_poll['question_votes'].'</td><td>'.$question_percentage.'</td><td><a href="mku_pollmanager.php?dopoll=modify_question&amp;questionid='.$get_poll['question_id'].'" title="modify question">Modify question</a></td></tr>';
												}
								...
								echo '<p>There is no questions in this poll for the moment <a href="mku_pollmanager.php?dopoll=add_question&amp;pollid='.$poll_id.'" title="click here to add one">Add a question</p>';
							}			
						}
						else{
					...
					default:
						...
						$query=mysql_query('SELECT * FROM mku_poll');
						$get_query=mysql_fetch_assoc($query);
						$num_query=mysql_num_rows($query);
						if($num_query==0){
							echo '<p style=""> Sorry but there is no polls for the moment<br/><br/><a href="mku_pollmanager.php?dopoll=newPoll" title="add new poll">Add new poll</a></p>';
						}
						else{			
							?>			
							<table style="100%;">
								<tr><td style="text-align:center;" colspan="3"><h1><a href="mku_pollmanager.php?dopoll=newPoll" title="add new poll">ADD NEW POLL</a></h1></td></tr>
								...
								<tr>
									...
									<td>
										<ol>
											<?php
											do{
												echo '<li><a href="mku_pollmanager.php?dopoll=viewpoll&amp;poll_id='.$get_query['poll_id'].'">'.$get_query['poll_name'].'</a></li>';

...
//--------- Debut ajout CI (cirr) ------
		if (!isset($rubrestreints[$id_auteur]) OR !in_array($row['id_rubrique'],$rubrestreints[$id_auteur])) {
//--------- Fin ajout CI ------
...
//--------- Debut ajout CI (cirr) ------
	// les rubriques affectees a l'auteur doivent jouer le role de secteur
	if (!$id_rubrique) {
		if ($id_auteur) {
			$result = sql_select("id_rubrique", "spip_auteurs_rubriques", "id_auteur=".$id_auteur);
			while ($row = sql_fetch($result)) {
				$rubrestreints[$id_auteur][] = $row['id_rubrique'];
			}
		}
		
		if ($rubrestreints[$id_auteur]) {
			$liste_rubriques = implode(",",$rubrestreints[$id_auteur]);
	
			$res = sql_select("rub1.id_rubrique, rub1.titre, rub1.id_parent, rub1.lang, rub1.langue_choisie, rub2.id_rubrique AS id_enfant",
					  "spip_rubriques AS rub1 LEFT JOIN spip_rubriques AS rub2 ON (rub1.id_rubrique = rub2.id_parent)",
					  "rub1.id_rubrique IN (".$liste_rubriques.")
					AND rub1.id_rubrique!=".sql_quote($exclu)."
					AND (rub2.id_rubrique IS NULL OR rub2.id_rubrique!=".sql_quote($exclu).")",  "", "0+rub1.titre,rub1.titre");
